...
- When Chris asks for detail on a specific item not in memory_context
  ...
  search_memories.
- When Chris asks about imported Claude or ChatGPT conversations (e.g.
  "what chats do I have?", "show me my Claude conversations"), use
  list_aichat to browse the list, then get_aichat for a specific one.
  Use search_memories with type=llm_chat only for keyword searches across
  the full history.
