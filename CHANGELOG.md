...
### Added
- **`get_code`, `list_changes`, `list_pending` LLM tool calls** (further addresses #129): the chat LLM can now drill into a specific code repo (`get_code` — call `list_projects(category='code')` first, then get detail by index), retrieve a project/goal activity digest (`list_changes [hours]` — slow, makes sub-LLM calls, only fires on explicit "what changed?" queries), and check the unified review inbox (`list_pending` — counts candidates, actions, and skill drafts). Extracted `_list_changes_text` from `cmd_changes` and `_list_pending_text` from `cmd_pending` so both the command and tool share the same code path. 7 new dispatch tests in `test_chat_tools.py`; `test_all_tool_names_in_dispatcher` updated; `skills/chat.md` updated with usage guidance for all three tools.
- **`list_notes`, `get_note`, `list_insights`, `list_aichat`, `get_aichat` LLM tool calls** (partially addresses #129): the chat LLM can now query Apple Notes, synthesis insights, and imported LLM chat history via natural language. Added `_get_note_text` (async, reads full note body from cache), `_list_insights_text` (extracted from `cmd_insights`), `_list_aichat_text`, and `_get_aichat_text` helpers in `chat_handler.py`. 9 new dispatch tests in `test_chat_tools.py`.
...
- **`search_memories type=llm_chat` returned "Unknown type filter"** — `_SEARCH_TYPE_FILTERS` had no `llm_chat` key, so the LLM guidance added in the previous cycle pointed at a filter the code rejected. Added `"llm_chat": {"llm_chat"}` to the filter map, added a `[platform]`-tagged format branch for llm_chat results in `_fmt_search_line`, and updated the error message's valid-types list. Added `test_search_memories_llm_chat_type_filter_returns_results` behavioral test.
...
- **Circles Phase D — in-Telegram rule editor**: `/circle_rule add <N> include|exclude <predicates>` and `/circle_rule remove <N> <rule_index>` commands let the host add or remove sync rules from a circle's ruleset YAML file without leaving Telegram. Predicates: `type:value`, `tags:v1,v2`, `tags_all:v1,v2`, `category:value`, `classification:value`, `hostname:value`, `source_title:text`. Edits are atomic (write-to-tmp + rename); the scanner picks up changes on its next cycle. Helpers `parse_rule_predicates()` and `write_ruleset_yaml()` added to `circle_ruleset.py`.
- **Circles Phase C (partial) — `CircleBotHandler`** (`circle_bot.py`): read-only Telegram command handler for per-circle member bots. Implements `/memories [N]`, `/search <query>`, `/events [N]`, `/commitments`, and `/help`, all scoped strictly to the circle's iCloud folder — MEMORIES_DIR is never accessed. Member allowlist enforcement: users not in `members[].telegram_user_id` receive a rejection message; empty members list allows access by bot-token obscurity. 26 unit tests including a MEMORIES_DIR isolation invariant test. Daemon wiring (per-circle Application startup), the `/ask` LLM command, and the FR-6 invite flow are deferred to a follow-up iteration.
- **Multi-project tracking for `/feature` and `/bug`** (#122): use `for:<project>` in the capture command to tag an item to a project (e.g. `/feature for:myapp Fix login`). The project name is stored in the `project:` frontmatter field and as a `project:<name>` GitHub label. `/features project:<name>` and `/features bug project:<name>` filter the list to one project. The list view shows `[project]` tags when items belong to different projects. Closes #122.
- **`concurrent_updates(True)` on Telegram application** (#116): slash commands from different users no longer block the LLM reply queue. Enables parallel command dispatch in the python-telegram-bot Application. Closes #116.
- `update_issue_priority` LLM tool: the chat skill can now update the priority of any bug or feature request via natural language (e.g. "set bug 2b2b14 to high priority", "mark the dark mode request as critical"). Identifies issues by 6-char short_id or title substring; syncs to GitHub when the issue has a `github_issue_number`. Closes #125.
- **Personal todos in morning briefing**: undated personal commitments (type `personal`, no `due_date`) are now shown in a "Personal todos" section of the daily briefing so items without a deadline are never buried. Items with a due date continue to appear only under "due today" / "overdue" to avoid duplication. Closes #120.

### Fixed
- `goals_tracker.py` / `chat_handler.py`: `GoalManager.list_goals_async()` and `list_projects_async()` use `MemoryCache.query_by_prefix()` (with disk fallback for newly-written files) instead of direct `glob()` + `read_text_with_retry()`. All Telegram command handlers (`/goals`, `/goal N`, `/addgoal`, `/projects`, `/project N`, `/addproject`, etc.) and LLM tool dispatches (`close_goal`, `close_project`, `list_goals`, `list_projects`) are now routed through the cache. Closes #149.
- `notes_scanner.py`: AppleScript now returns `YYYY-MM-DDTHH:MM:SS` instead of `YYYY-MM-DD` for the note modification timestamp — intra-day edits after the first scan were silently ignored because the date-only comparison matched even though the note had changed (#146).
- `notes_scanner.py`: `_write_memory` now returns `True`/`False` instead of `None`; the scan loop only records a note as processed in state when the return value is `True` — previously a silent write failure (caught internally) still marked the note as processed, suppressing retries (#147).
- `notes_scanner.py`: `skip_folders` now uses substring matching (`any(s in folder_lower for s in skip_folders)`) instead of exact set membership — a configured value of `"Archive"` now correctly excludes folders like "Work Archive" as the docs describe (#148).
- `contact_tracker.py`: second-scan contact files were corrupted — `MemoryCache.body` contains the full file text (frontmatter + body), so reading it verbatim as the new body embedded the old frontmatter block inside the markdown section. Fixed by splitting on `---` to extract only the post-frontmatter portion before reusing it. Regression test added. Closes #145.
- `chat_handler.py`: `_close_goal_text` and `_close_project_text` now guard against empty title strings (fixes #143, #144 — empty string matched all goals/projects, closing a random one when only one existed). Also detect and report no-op when the goal/project is already at the target status instead of returning a misleading success message (fixes #142).
- `quota_scrapers.py`: `scrape_claude` and `scrape_chatgpt` now return `{}` immediately when the cookie file does not exist, preventing spurious `NotImplementedError` raises for unconfigured scrapers (#114). Four unit tests added in `tests/unit/test_quota_scrapers.py`.
- `install.sh`: new pip-audit step runs after every `./install.sh` to scan the deploy venv for known CVEs. Non-blocking — prints a warning on findings but does not abort the install (#113).
...
### Fixed
- `test_related_memories_recency_filter` hardcoded `last_checked = "2026-05-01T00:00:00"` as a "future" cutoff; the test broke after that date passed — replaced with `datetime.now() + timedelta(hours=1)` (#115).
...
- Heartbeat / healthcheck for all async loops (`heartbeat.py`): each scanner calls `record_beat(loop_name, status, error)` after every scan iteration, writing `heartbeat-{hostname}.json` to `BRAIN_DIR` so the full node can read across all connected instances (#10). New `/status` Telegram command reads all heartbeat files and reports last-run time, OK/ERR status, and error snippets per loop per machine, flagging instances with no heartbeat in 10+ minutes as `[STALE]`.
...
- `llm_routes.resolve()` is now provider-aware: non-SkillExecutor code paths (index builder, commitment tracker, code/email/calendar/slack scanners, goal/project agent) now honour `SECOND_BRAIN_PROVIDER` and route to the appropriate model family — `gemini/gemini-2.0-flash` for `gemini`, `openai/gpt-4o[-mini]` for `openai`, Claude models for `claude`/`both` (default). Previously these paths always used Claude regardless of the configured provider (#53).
...
- `/remember <url> [quick|standard|deep]`: optional depth control for manual URL captures. `quick` (alias `1`) uses a new concise `summarize-webpage-quick` skill (1-2 sentence summary + 3 bullets); `standard` (alias `2`, default) uses the existing auto-detect routing; `deep` (alias `3`) uses `summarize-webpage-detailed` for rich notes — the same as `/note`. Numeric aliases `1`/`2`/`3` are accepted for fast typing (#22).
...
- `email_scanner`: existing threads misclassified before the PR #98 prompt fix are now reclassified on the next scan cycle. A `CLASSIFIER_VERSION` constant (currently `2`) is written to each email memory's frontmatter; threads whose stored version doesn't match are treated as needing update even when `message_count` and `last_message` are unchanged (#99).
...
- `chat_handler`: `/status` command used an f-string with a backslash in the path-scrubber expression, which is a `SyntaxError` on Python 3.9. Fixed by extracting the `re.sub()` call to a local variable (#10).
...
- `/bug` and `/feature` no longer discard the local memory file when GitHub backing is enabled: both commands now write a `feature-request-*.md` file to `memories/` (with `github_issue_number:` set) in addition to creating the GitHub issue, so bugs remain visible to the memory/context system. `/feature_import` likewise stops moving promoted files to `memories/archive/` — they stay in place with `github_issue_number:` stamped in their frontmatter so future imports skip them (#78).
- `close_issue` LLM tool now syncs status to the GitHub issue before updating the local file when the file carries a `github_issue_number`, preventing local/GitHub state divergence. If the GitHub call fails, the local file is left unchanged and an error is returned (#83).
- `scripts/promote_local_features.py` now keeps promoted files in `memories/` (instead of moving them to `memories/archive/`) to match the behaviour of `cmd_feature_import` in `chat_handler.py` (#83).
- `memory_cache.query_by_prefix()` in enabled (SQLite) mode now strips a trailing `-` from the prefix argument before the `WHERE prefix =` comparison, so callers using either `"calendar-event-"` or `"calendar-event"` style get identical results (#83).
