...
### Added
- **`/prs` command + `list_prs` LLM tool** (partial #127): the Telegram bot can now list open (or closed/all) pull requests from the configured GitHub repository via `/prs [open|closed|all]` or by asking the LLM "show me open PRs". Useful for monitoring babysitter-loop activity. `list_pull_requests()` added to `GitHubClient`; `_list_prs_text()` and `cmd_prs()` added to `chat_handler.py`; `list_prs` schema and dispatch added to `chat_tools.py`. 9 new tests in `test_github_client.py`, `test_chat_tools.py`, `test_chat_handler.py`, and `test_e2e_features.py`.
