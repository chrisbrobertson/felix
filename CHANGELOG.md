...
### Added
- **`get_quota`, `report_quota`, `reset_quota` LLM tool calls** (partially addresses #129): the chat LLM can now query Claude.ai Pro and ChatGPT Plus quota status, self-report usage ("I've used 23/40 Claude messages"), and clear stale state when a window resets. `_quota_status_text`, `_report_quota_text`, and `_reset_quota_text` helpers added to `TelegramChatHandler`; `report_quota` and `reset_quota` added to `MUTATING_TOOLS`. 9 new tests across `test_chat_tools.py` and `test_chat_handler.py`. Skills/chat.md updated with quota tool guidance.
