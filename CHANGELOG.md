...
### Added
- **`get_status`, `get_usage`, `list_skill_drafts`, `get_skill_draft` LLM tool calls** (partially addresses #129): the chat LLM can now query daemon heartbeat/loop health, LLM token usage stats, and pending skill optimizer drafts via natural language. Extracted `_status_text()`, `_usage_text()`, `_list_skill_drafts_text()`, and `_get_skill_draft_text()` helpers from their respective `cmd_*` handlers; the commands now delegate to these helpers. 12 new dispatch and schema-presence tests in `test_chat_tools.py`. Updated `skills/chat.md` with tool-selection guidance.
