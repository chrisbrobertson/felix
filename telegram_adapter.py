...
class TelegramAdapter:
    ...
    async def send_text(self, chat_id: str, text: str) -> None:
        ...
        bot = self._bot()
        if bot is None:
            log.warning("TelegramAdapter.send_text: bot not available")
            return
        for chunk in _chunk_text(text, _MAX_CHUNK):
            try:
                await bot.send_message(chat_id=int(chat_id), text=chunk)
            except Exception as e:
                log.warning("TelegramAdapter.send_text failed: %s", e)
    ...
    def get_chat_id(self) -> str | None:
        ...
        if chat_id is not None:
            return str(chat_id)
        # Fallback: read from notification state
        try:
